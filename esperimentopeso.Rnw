...
\end{itemize}
ed anzi le due variabili sono quasi perfettamente indipendenti, in quanto Pr = 0,83515: c’è il 83,51\% di probabilità che sia vera l’ipotesi nulla.
...
\\
Per avere una visualizzazione migliore di come incidano i fattori sul quantità di caffè fuoriuscita, si utilizzano i grafici di interazione degli effetti [\ref{fig:subfig}]. 
%
<<echo=F,fig=T,include=FALSE>>=
...
\item L'effeto dei fattori C e D presentano un'elevata interazione; al contrario dimostrano scarsa interazione ed efetto elevato se confrontati con i fattori E e D.
\end{itemize}
%
\section{Piano fattoriale frazionato}
\label{sec:ffp1}
Si può utilizzare un piano frazionato $2^{5-1}_V$ con una relazione delt tipo \textbf{I = ABCDE} ponendo poi il fattore \textbf{E} in funzione degli altri secondo la realzione \textbf{E = ABCD}. Questo permette di ottenere un numero di effettuare metà delle prove rispetto le misurazioni necessarie per un piano fattoriale completo.
...
@
\end{figure}
Anche in questo caso non si hanno repliche e non è possibile eseguire l'analisi \textsc{anova}, quindi si cercano gli effetti più significativi.
<<>>=
...
@
\end{figure}
\newline Da cui possiamo osservare in risultato simile a quanto già osservato nell'analisi del piano fattoriale completo, ovvero l'influenza maggiore è data da \textbf{C}.
