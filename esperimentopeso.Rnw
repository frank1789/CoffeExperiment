...
\end{figure}\\\\
Modifichiamo il modello lineare escludendo gli effetti che non sono significativi, cioè risentono solo dell'errore sperimentale e sono quindi distribuiti lungo la retta normale, visti in figua \ref{fig:EffectsQuantile1}. Il nuovo modello lineare assume la relazione ($Yield \sim A * C * D + B * E$), si ricalcola il modello \textsc{anova} quindi si procede alla verifica adeguatezza verificando normalità dei residui e assenza di pattern.
<<result=>>=
...
\end{figure}
In figura \ref{fig:qunatilePlotResidual} si osserva una distribuzione non propriamente normale quindi si modifica il modello escudendo quei fattori prima considerati, ma vicini alla retta.
Quindi si assumeuna relazione ($Yield \sim A * C + B * E$) e si performa \textsc{anova}:
%# linear model 3
<<>>=
...
\end{figure}
Ulteriore conferma ci viene data eseguendo lo Shapiro-Test sul modello:
<<>>=
shapiro.test(df.lm3$residuals)
@
Ottenendo così un valore del $4,9\%$ di commettere un errore di tipo I.