\chapter{Analisi}
Attraverso l'uso di R si è realizzata la matrice degli esperimenti, \emph{Design Matrix}, nella quale si realizzano le combinazioni dei fattori prima descritti e i relativi livelli. Dopo aver realizzato la matrice degli esperimenti come visto in precedenza. Sono stati realizzate le prove.
...
\end{figure}
%
\newpage \noindent In figura \ref{fig:qunatilePlotResidual} nessuno dei diagrammi sembra mostrare una tipica distribuzione normale dei residui e il boxplot \ref{fig:boxplotpeso} indica che potrebbero esserci outliers.
...
Come verifica finale si utilizza anche il test di Shapiro, che conferma le conclusioni ottenute con
una probabilità di errore di tipo I pari al $5\,\%$.
<<>>=
