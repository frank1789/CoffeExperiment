...
df
@
Definendo un modello lineare espresso dalla seguente relazione $(Yield \sim A*B*C*D*E)$ si ottiene come osservato per l'esperimento precedente che i residui sono nulli, per l'ipotesi già osservate di piano fattoriale completo e assenza di replicazioni è necessario utilizzare il metodo di Daniel per individuare l'effeto dei fattori, %
quindi applicando il metodo di Daniel  si osserva il grafico quantile--quantile per il modello lineare completo:
<<echo=F,results=hide>>=
...
\begin{figure}[htb]
\centering
  \includegraphics[width=0.80\textwidth]{Report-024}
\caption{da inserire}
...
\begin{figure}[h]
\centering
  \includegraphics[width=0.70\textwidth]{Report-027}
