...
Come fatto in precedenza, nella sezione (\ref{sec:ffp1}), si genera un piano frazionato $2^{5-1}_V$ con le medesime premesse e ottenendo i vantaggi prima evidenziati, si importa design matrix:
<<result=hide>>=
...
<<>>=
df.ffp.lm2 <- lm(log(Yield) ~ A + C * E, data = df.ffp)
anova(df.ffp.lm2)
@
<<echo=F,results=hide>>=
options(SweaveHooks = list(fig = function() par(mfrow=c(2,2)))) 
